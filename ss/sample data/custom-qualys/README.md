# Custom Qualys Test Pack

This folder contains only synthetic or generated test data. It contains no customer data.

## Folder contents

- `asset_inventory/custom-qualys_asset_inventory.csv`: 100-asset inventory. Assets found in the supplied Custom Qualys scans are included so inventory and scanner analysis can be tested together.
...
- `scan_results/`: Custom Qualys ad hoc and monthly scanner exports using the supported raw field layout.
...
2. Import `asset_inventory/custom-qualys_asset_inventory.csv` in Asset inventory.
...
5. Select Custom Qualys in Tool selection.
6. For Monthly comparison, upload the four files listed in `manifest.json > monthlyFiles`.
7. For Ad hoc analysis, upload `scan_results/custom_qualys_adhoc_july_2026.csv`.
