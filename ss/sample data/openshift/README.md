# Red Hat OpenShift Test Pack

This folder contains only synthetic or generated test data. It contains no customer data.

## Folder contents

- `asset_inventory/openshift_asset_inventory.csv`: 100-asset inventory. Assets found in the supplied Red Hat OpenShift scans are included so inventory and scanner analysis can be tested together.
...
- `scan_results/`: Red Hat OpenShift ad hoc and monthly scanner exports using the supported raw field layout.
...
2. Import `asset_inventory/openshift_asset_inventory.csv` in Asset inventory.
...
5. Select Red Hat OpenShift in Tool selection.
6. For Monthly comparison, upload the four files listed in `manifest.json > monthlyFiles`.
7. For Ad hoc analysis, upload `scan_results/openshift_july_2026_100plus.csv`.
