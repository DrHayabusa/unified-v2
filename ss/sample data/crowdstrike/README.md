# CrowdStrike Exposure Management Test Pack

This folder contains only synthetic or generated test data. It contains no customer data.

## Folder contents

- `asset_inventory/crowdstrike_asset_inventory.csv`: 100-asset inventory. Assets found in the supplied CrowdStrike Exposure Management scans are included so inventory and scanner analysis can be tested together.
...
- `scan_results/`: CrowdStrike Exposure Management ad hoc and monthly scanner exports using the supported raw field layout.
...
2. Import `asset_inventory/crowdstrike_asset_inventory.csv` in Asset inventory.
...
5. Select CrowdStrike Exposure Management in Tool selection.
6. For Monthly comparison, upload the four files listed in `manifest.json > monthlyFiles`.
7. For Ad hoc analysis, upload `scan_results/crowdstrike_vulnerability_per_asset_july_2026_100plus.csv`.
