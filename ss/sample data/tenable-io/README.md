# Tenable.io Test Pack

This folder contains only synthetic or generated test data. It contains no customer data.

## Folder contents

- `asset_inventory/tenable-io_asset_inventory.csv`: 100-asset inventory. Assets found in the supplied Tenable.io scans are included so inventory and scanner analysis can be tested together.
...
- `scan_results/`: Tenable.io ad hoc and monthly scanner exports using the supported raw field layout.
...
2. Import `asset_inventory/tenable-io_asset_inventory.csv` in Asset inventory.
...
5. Select Tenable.io in Tool selection.
6. For Monthly comparison, upload the four files listed in `manifest.json > monthlyFiles`.
7. For Ad hoc analysis, upload `scan_results/tenable_io_july_2026_100plus.csv`.
