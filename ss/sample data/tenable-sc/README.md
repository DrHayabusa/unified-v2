# Tenable.sc Test Pack

This folder contains only synthetic or generated test data. It contains no customer data.

## Folder contents

- `asset_inventory/tenable-sc_asset_inventory.csv`: 100-asset inventory. Assets found in the supplied Tenable.sc scans are included so inventory and scanner analysis can be tested together.
...
- `scan_results/`: Tenable.sc ad hoc and monthly scanner exports using the supported raw field layout.
...
2. Import `asset_inventory/tenable-sc_asset_inventory.csv` in Asset inventory.
...
5. Select Tenable.sc in Tool selection.
6. For Monthly comparison, upload the four files listed in `manifest.json > monthlyFiles`.
7. For Ad hoc analysis, upload `scan_results/tenable_sc_july_2026_100plus.csv`.
